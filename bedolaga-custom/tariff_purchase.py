...
from app.utils.keys_photo import edit_keys_photo
...
HIDDEN_PURCHASE_TARIFFS = {'Белые списки'}
...
def _format_devices(count: int) -> str:
...
def _format_rub_price(kopeks: int) -> str:
    return f'{kopeks // 100} руб.'


def _tariff_emoji(tariff: Tariff) -> str:
    return TARIFF_EMOJI.get(tariff.name, '📦')


def _filter_purchasable_tariffs(tariffs: list[Tariff]) -> list[Tariff]:
    return [t for t in tariffs if t.name not in HIDDEN_PURCHASE_TARIFFS]


async def _prompt_paid_tariff_upgrade(
    ...
    *,
    ...
    """Экран выбора платного тарифа вместо продления бесплатного."""
    from app.utils.satka_subscription_policy import FREE_TARIFF_DENIED_MESSAGE, tariff_is_free

    promo_group_id = getattr(db_user, 'promo_group_id', None)
    tariffs = _filter_purchasable_tariffs(await get_tariffs_for_user(db, promo_group_id))
    paid_tariffs = [
        t for t in tariffs if t.is_active and not t.is_daily and not tariff_is_free(t.id, t)
    ]
    if not paid_tariffs:
        await callback.answer('Нет доступных платных тарифов', show_alert=True)
        return

    keyboard = [
        [InlineKeyboardButton(text=f'📦 {t.name}', callback_data=f'tariff_select:{t.id}')]
...
def _build_period_button(
    ...
    if discount_percent > 0:
        price_kopeks = _apply_promo_discount(price_kopeks, group_pct, offer_pct)

    discount_label = PERIOD_DISCOUNT_LABELS.get(period, '')
    if discount_percent > 0:
        discount_label = f' 🔥−{discount_percent}%'

    button_text = f'📅 {period} д. - {_format_rub_price(price_kopeks)}{discount_label}'
    return InlineKeyboardButton(text=button_text, callback_data=f'tariff_period:{tariff.id}:{period}')


def _build_period_keyboard_rows(
    tariff: Tariff,
    callback_prefix: str,
    db_user: User | None = None,
    ...
    for period_str in sorted(prices.keys(), key=int):
        ...
        button = _build_period_button(tariff, period, price, db_user)
        if callback_prefix == 'tariff_period_traffic':
    ...
    for index in range(0, len(period_buttons), 2):
        rows.append(period_buttons[index : index + 2])
    return rows
...
def format_tariffs_list_text(
    ...
    """Шаг 1/2 — выбор тарифа."""
    lines = [
        '🔑 <b>Шаг 1/2: Выбор тарифа</b>',
        '',
        'Выберите подходящий тариф для будущего ключа:',
    ]
    if has_period_discounts:
        lines.append('')
        lines.append('🎁 <i>Для вас доступны дополнительные скидки</i>')
    return '\n'.join(lines)
...
def get_tariffs_keyboard(
    ...
    """Клавиатура выбора тарифов 2×2 с эмодзи."""
    if purchased_tariff_ids is None:
        purchased_tariff_ids = set()

    tariff_buttons: list[InlineKeyboardButton] = []
    for tariff in tariffs:
        emoji = _tariff_emoji(tariff)
        mark = ' ✅' if tariff.id in purchased_tariff_ids else ''
        tariff_buttons.append(
            InlineKeyboardButton(
                text=f'{emoji} {tariff.name}{mark}',
                callback_data=f'tariff_select:{tariff.id}',
            )
        )

    buttons: list[list[InlineKeyboardButton]] = []
    for index in range(0, len(tariff_buttons), 2):
        buttons.append(tariff_buttons[index : index + 2])

    buttons.append([InlineKeyboardButton(text='↩️ Вернуться назад', callback_data='back_to_menu')])
...
def get_tariff_periods_keyboard(
    ...
    """Клавиатура выбора периода 2×2 в стиле SATKAVPN."""
    buttons = _build_period_keyboard_rows(tariff, 'tariff_period', db_user)
    buttons.append([InlineKeyboardButton(text='↩️ Вернуться назад', callback_data='tariff_list')])
    return InlineKeyboardMarkup(inline_keyboard=buttons)
...
def get_tariff_periods_keyboard_with_traffic(
    ...
    """Клавиатура выбора периода для тарифа с кастомным трафиком."""
    buttons = _build_period_keyboard_rows(tariff, 'tariff_period_traffic', db_user)
    buttons.append([InlineKeyboardButton(text='↩️ Вернуться назад', callback_data='tariff_list')])
    return InlineKeyboardMarkup(inline_keyboard=buttons)
...
def format_tariff_info_for_user(
    ...
    """Шаг 2/2 — информация о тарифе и выбор срока."""
    emoji = _tariff_emoji(tariff)
    traffic_line = 'Безлимитно' if tariff.traffic_limit_gb == 0 else format_traffic(tariff.traffic_limit_gb)

    info_lines = [
        f'├ Лимит устройств: {_format_devices(tariff.device_limit)}',
        f'├ Лимит трафика: {traffic_line}',
    ]
    info_lines[-1] = info_lines[-1].replace('├', '└', 1)

    text = f"""{emoji} <b>Шаг 2/2: Выбор срока для тарифа «{html.escape(tariff.name)}»</b>
...
{chr(10).join(info_lines)}"""
    ...
    if not is_daily:
        text += '\n\nВыберите подходящий срок тарифа для вас:'
...
async def show_tariffs_list(
    ...
    from app.utils.satka_subscription_policy import (
        filter_purchase_tariffs,
        should_hide_free_tariff,
    )

    if await should_hide_free_tariff(db, db_user.id):
        tariffs = filter_purchase_tariffs(tariffs, hide_free=True)
    ...
    if settings.is_multi_tariff_enabled():
        ...
        active_subs = await get_active_subscriptions_by_user_id(db, db_user.id)
        purchased_tariff_ids = {s.tariff_id for s in active_subs if s.tariff_id}
    ...
    await edit_keys_photo(callback, tariffs_text, keyboard)
    await callback.answer()
...
async def select_tariff(
    ...
    from app.utils.satka_subscription_policy import ensure_tariff_purchase_allowed

    allowed, deny_msg = await ensure_tariff_purchase_allowed(db, db_user.id, tariff_id)
    if not allowed:
        await callback.answer(deny_msg, show_alert=True)
        return
    ...
    if settings.is_multi_tariff_enabled():
        ...
        _existing = next((s for s in _active if s.tariff_id == tariff_id), None)
        if _existing:
    ...
    if is_daily:
        ...
        if can_custom_days:
            ...
            await edit_keys_photo(
                callback,
                period_text,
                get_tariff_periods_keyboard_with_traffic(tariff, db_user.language, db_user=db_user),
            )
        else:
            await edit_keys_photo(
                callback,
                format_tariff_info_for_user(tariff, db_user.language),
                get_tariff_periods_keyboard(tariff, db_user.language, db_user=db_user),
            )
...
async def confirm_tariff_purchase(
    ...
    from app.utils.satka_subscription_policy import (
        after_successful_tariff_purchase,
    ...
    if existing_sub and existing_sub.tariff_id == tariff.id:
        from app.utils.satka_subscription_policy import ensure_tariff_extend_allowed

        allowed, deny_msg = await ensure_tariff_extend_allowed(
            db, db_user.id, tariff.id, tariff=tariff
        )
        if not allowed:
            await callback.answer(deny_msg, show_alert=True)
            return
    ...
    saved_promo_expires = getattr(db_user, 'promo_offer_discount_expires_at', None) if consume_promo else None
    try:
        from app.utils.satka_subscription_policy import is_free_tariff
        ...
        success = await subtract_user_balance(
            ...
            consume_promo_offer=consume_promo,
            mark_as_paid_subscription=final_price > 0 and not is_free_tariff(tariff.id),
        )
        if not success:
            try:
                await callback.message.edit_text('❌ Ошибка списания баланса')
...
async def show_tariff_extend(
    ...
    from app.utils.satka_subscription_policy import ensure_tariff_extend_allowed, tariff_is_free

    if tariff_is_free(tariff.id, tariff):
        await _prompt_paid_tariff_upgrade(callback, db_user, db, back_callback='menu_subscription')
        return
...
async def select_tariff_extend_period(
    ...
    allowed, deny_msg = await ensure_tariff_extend_allowed(
        db, db_user.id, tariff_id, tariff=tariff
    )
    if not allowed:
        await callback.answer(deny_msg, show_alert=True)
        if len(parts) < 4:
            await _prompt_paid_tariff_upgrade(
                callback, db_user, db, back_callback='menu_subscription'
            )
        return
...
async def confirm_tariff_extend(
    ...
    from app.utils.satka_subscription_policy import ensure_tariff_extend_allowed, is_free_tariff
    ...
    try:
        ...
        try:
            ...
            await admin_notification_service.send_subscription_purchase_notification(
                ...
                None,  # Транзакция отсутствует, оплата с баланса
                period,
                was_trial_conversion=was_trial and final_price > 0 and not is_free_tariff(tariff.id),
                amount_kopeks=final_price,
