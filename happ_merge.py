...
BEELINE_WHITELIST_REMARK = "Лучший обход! БС CDN"
BEELINE_NATIVE_KEY = "Германия Beeline"
...
BEELINE_CDN_BLOB_MARKERS = (
    "lunz3cg8gz.a.trbcdn.net",
...
def is_beeline_native_item(item: dict) -> bool:
    ...
    if "бс cdn" in remark.lower():
        return True
    blob = remark + json.dumps(item, ensure_ascii=False)
    return any(marker in blob for marker in BEELINE_CDN_BLOB_MARKERS)


def find_all_beeline_cdn_natives(items: list) -> list[dict]:
    seen: set[str] = set()
    out: list[dict] = []
    for item in items:
        if not is_beeline_native_item(item):
            continue
        remark = (item.get("remarks") or "").strip()
        key = remark or json.dumps(item, ensure_ascii=False, sort_keys=True)
        if key in seen:
            continue
        seen.add(key)
        out.append(item)
...
def subscription_vless_uuid_from_satka(satka: list) -> str | None:
    ...
    for item in satka:
        if is_beeline_native_item(item):
            continue
...
def resolve_all_beeline_cdn_natives(
    natives: dict[str, dict],
    *,
    satka: list | None = None,
) -> list[dict]:
    """Все CDN-хосты (Beeline + Yandex) из native подписки или template."""
    seen: set[str] = set()
    out: list[dict] = []

    def add(item: dict | None) -> None:
        if not item or not is_beeline_native_item(item):
            return
        remark = (item.get("remarks") or "").strip()
        key = remark or json.dumps(item, ensure_ascii=False, sort_keys=True)
        if key in seen:
    ...
    if satka:
        for item in find_all_beeline_cdn_natives(satka):
            add(item)
...
def prepare_beeline_whitelist_cfg(native_cfg: dict) -> dict:
    """CDN tunnel (Beeline/Yandex) — без YouTube adblock routing, remark из native."""
...
def append_beeline_whitelist(
    ...
    *,
    ...
    if not BEELINE_IN_SUBSCRIPTION:
        return
    natives_list = resolve_all_beeline_cdn_natives(natives, satka=satka)
    if not natives_list:
        return
    uid = subscription_vless_uuid_from_satka(satka) if satka else None
    existing_remarks = {
        (c.get("remarks") or "").strip().lower()
        for c in result
        if is_beeline_cfg(c)
    }
    for native in natives_list:
        ...
        result.append(cfg)
        existing_remarks.add(remark.lower())
...
def ensure_beeline_whitelist_entry(
    ...
    *,
    ...
    """Добавить все CDN-хосты (Beeline + Yandex), если ещё не в списке."""
...
def build_free_extra_servers(token: str) -> list:
    ...
    if len(wl_items) > FREE_EXTRA_WHITELIST_INDEX:
        extra_cfg = prepare_whitelist_cfg(
            wl_items[FREE_EXTRA_WHITELIST_INDEX],
            FREE_EXTRA_WHITELIST_NO,
            wl_items,
            natives=wl_natives,
        )
        if not is_beeline_cfg(extra_cfg):
