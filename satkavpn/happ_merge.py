...
# Платная подписка: candelix | sinful (бесплатные extras остаются на sinful)
...
def fetch_paid_whitelist_json() -> tuple[dict, ...]:
    """Белые списки для платной подписки (по умолчанию Candelix)."""
...
def paid_whitelist_from_candelix() -> bool:
    return PAID_WHITELIST_SOURCE != "sinful"
...
def fetch_whitelist_mihomo_proxies() -> dict[int, dict]:
    ...
    for i, item in enumerate(items, 1):
        number = candelix_whitelist_number(item, i) if use_candelix else i
        if use_candelix:
...
# Регионы для «Белых списков» (RU, DE, NL, FR… — как у sinful LTE)
...
def whitelist_flag_for_item(
    ...
    """Флаг региона: у Candelix в апстриме везде 🇪🇺 — подставляем по номеру."""
    if candelix:
...
def append_whitelist_subset(
    ...
    *,
    ...
    for item in items:
        server_no += 1
        number = (
            candelix_whitelist_number(item, server_no)
            if use_candelix
            else server_no
        )
...
def append_whitelist_paid(
    ...
    """Белые списки в платной подписке (Candelix или sinful по PAID_WHITELIST_SOURCE)."""
    use_candelix = paid_whitelist_from_candelix()
    items = fetch_paid_whitelist_json()
    prepared: dict[int, dict] = {}
    for item in items:
